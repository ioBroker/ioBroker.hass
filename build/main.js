...
class HassAdapter extends adapter_core_1.Adapter {
    ...
    isEntityExcluded(entityId) {
        return (0, entityFilter_1.isEntityIdExcluded)(entityId, this.excludeFilter);
    }
    isObjectIdExcluded(id) {
        const path = id.startsWith(`${this.namespace}.`) ? id.substring(this.namespace.length + 1) : id;
        return (0, entityFilter_1.isObjectPathExcluded)(path, this.excludeFilter);
    }
    ...
    async parseStates(entities, services) {
        ...
        if (excludedObjectCount > 0) {
            this.log.info(`Entity filter excluded ${excludedObjectCount} single object${excludedObjectCount === 1 ? '' : 's'} from sync`);
        }
     ...
     */
    async replayPendingStateChanges() {
    ...
    async onHassStateChanged(entity) {
        if (this.isEntityExcluded(entity.entity_id)) {
    ...
    async main() {
        ...
        this.excludeFilter = (0, entityFilter_1.buildExcludeFilter)(stringPatterns, this.namespace);
        if (stringPatterns.length === 0) {
            this.log.info('Entity filter inactive (no exclude patterns configured)');
        }
        else {
            this.log.info(`Entity filter active: ${stringPatterns.length} pattern(s) loaded: ${stringPatterns.join(', ')}`);
        }
